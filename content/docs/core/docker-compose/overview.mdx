...
Deployi integrates with Docker Compose and Docker Stack to provide flexible deployment solutions. Whether you are developing locally or deploying at scale, Deployi facilitates application management through these powerful Docker tools.

### Configuration Methods

Deployi provides two methods for creating Docker Compose configurations:
...
A code editor within Deployi allows you to specify environment variables for your Docker Compose file. By default, Deployi creates a `.env` file in the specified Docker Compose file path.

###  Monitoring

Monitor each service individually within Deployi. If your application consists of multiple services, each can be monitored separately to ensure optimal performance.

### Logs

Access detailed logs for each service through the Deployi log viewer, which can help in troubleshooting and ensuring the stability of your services.
...
- **Command**: Deployi has a defined command to run the Docker Compose file, ensuring complete control through the UI. However, you can append flags or options to the command.
